...
export default function QueueMonitor() {
    ...
    const queuePosition = orders.filter(o => {
        if (activeOrder.status === 'READY') {
            return o.status === 'READY' && parseInt(o.id) < parseInt(activeOrder.id);
        }
        return o.status !== 'CLAIMED' && parseInt(o.id) < parseInt(activeOrder.id);
    }).length + 1;
    ...
    return (
        <div className="w-full animate-in slide-in-from-top duration-700">
            <div className={`rounded-[2.5rem] p-8 text-white shadow-2xl transition-all duration-500 relative overflow-hidden border ${
                ...
                <div className={`absolute -top-24 -right-24 w-64 h-64 blur-[100px] rounded-full animate-pulse ${
                    activeOrder.status === 'READY' ? 'bg-white/20' : 'bg-emerald-500/10'
                }`} />
                ...
                <div className="relative z-10 flex flex-col lg:flex-row justify-between items-center gap-8">
                    ...
                    <div className="flex flex-col items-center lg:items-start gap-4">
                        ...
                        <div className="text-center lg:text-left">
                            ...
                            <p className={`text-xs font-bold uppercase tracking-widest ${
                                activeOrder.status === 'READY' ? 'text-emerald-100/60' : 'text-slate-400'
                            }`}>
                    ...
                    <div className="grid grid-cols-2 gap-4 w-full lg:w-auto">
                        <div className={`p-4 rounded-3xl border text-center ${
                            activeOrder.status === 'READY' ? 'bg-white/10 border-white/10' : 'bg-white/5 border-white/10'
                        }`}>
                    ...
                    <div className="w-full lg:max-w-xs">
                        <div className={`p-6 rounded-[2rem] border backdrop-blur-md transition-all ${
                            activeOrder.status === 'READY'
                            ...
                            <div className="flex items-start gap-4">
                                <div className={`w-10 h-10 rounded-2xl flex items-center justify-center shrink-0 ${
                                    activeOrder.status === 'READY' ? 'bg-white text-emerald-600' : 'bg-slate-700 text-slate-400'
                                }`}>
